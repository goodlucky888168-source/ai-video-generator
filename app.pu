...
def _render_api_check_panel():
    ...
    with st.expander("🔧 API 金鑰檢查", expanded=False):
        ...
        api_checks = {
            ...
            "Kling Access": {
                "key": keys.get("kling_access"),
                "required": True,
                "format": "alphanumeric"
            },
            "Kling Secret": {
                "key": keys.get("kling_secret"),
                "required": True,
                "format": "alphanumeric"
            },
            "ElevenLabs": {
            ...
            "Beatoven": {
                "key": keys.get("beatoven"),
                "required": False,
                "format": "alphanumeric"
            },
        }
        
        for name, info in api_checks.items():
            value = info["key"]
            ...
            if value:
                ...
                status = "✅" if required else "✅ (可選)"
                st.success(f"{status} {name}: {masked}")
            else:
                status = "❌ (必需)" if required else "⚠️ (可選)"
                st.warning(f"{status} {name}: 未設定")
        
        st.divider()
        
        # 測試 OpenAI
        st.markdown("### 🧪 測試 OpenAI")
        if st.button("測試 OpenAI 連接", key="test_openai"):
            try:
                ...
                st.success("✅ OpenAI 連接成功！")
                st.json(result)
            
            except Exception as e:
                st.error(f"❌ OpenAI 測試失敗：{str(e)}")
