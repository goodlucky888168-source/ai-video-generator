...
# ==================== 分鏡生成 ====================
def generate_storyboard(user_input: str, characters: list, openai_key: str) -> list:
    char_desc = ""
...
def main():
    ...
    if mode == "🎥 單場景模式":
        ...
        with col_input:
            ...
            with col_opt1:
                video_duration = st.selectbox("影片長度", [5, 10], index=0)
            with col_opt2:
    ...
    # ==================================================
                ...
                if not char_name:
                    ...
                    with st.spinner("分析角色外觀中..."):
                        try:
                            ...
                            char_desc = desc_result.get("narration", "角色外觀描述")
        ...
        # -------- 劇情輸入 --------
        st.subheader("📝 Step 3：輸入劇情")
        user_input = st.text_area(
            "描述故事劇情",
            placeholder="例如：小明在公園遇見小花，兩人一起看夕陽，最後依依不捨地道別...",
            height=160,
            key="multi_input"
        )
        ...
        with col_dur:
            video_duration = st.selectbox("每個分鏡長度", [5, 10], index=0, key="multi_dur")
        with col_ratio:
            aspect_ratio = st.selectbox("畫面比例", ["16:9", "9:16", "1:1"], index=0, key="multi_ratio")
        ...
        if st.session_state.storyboard:
            ...
            for i, s in enumerate(st.session_state.storyboard):
                with st.expander(f"分鏡 {i+1}｜角色：{', '.join(s.get('characters', []))}"):
                    st.write(f"**場景：** {s.get('scene', '')}")
                    st.write(f"**旁白：** {s.get('narration', '')}")
                    st.write(f"**時長：** {s.get('duration', 5)} 秒")
            ...
            def generate_scene(scene, idx):
                try:
                    ...
                    url = generate_video(
                        ...
                        ref_img_b64,
                        progress_cb,
                        scene.get("duration", video_duration),
                        aspect_ratio
                    )
