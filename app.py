...
def main():
    ...
    if mode == "🎥 單場景模式":
        ...
        if st.button("🚀 開始生成", type="primary", use_container_width=True, key="single_gen"):
            ...
            narration = result.get("narration", "").strip()   # ✅ Bug 3 修正
    ...
    # ==================================================
            ...
            if submitted:
                if not char_name:
                    ...
                    with st.spinner("分析角色外觀中..."):
                        try:
                            # ✅ Bug 4：立刻轉 base64，避免 UploadedFile 失效
                            all_b64 = []
        ...
        if st.button("🚀 開始生成所有分鏡影片", type="primary", use_container_width=True, key="multi_gen"):
            ...
            # ✅ Bug 2：快照，避免 Thread 閉包引用問題
            chars_snapshot  = [dict(c) for c in selected_chars]
            boards_snapshot = get_edited_storyboard()   # ✅ 讀取編輯後的分鏡
            ...
            # ✅ Bug 2 修正：用 collected dict 收集結果，不重複呼叫 .result()
            collected = {}
            ...
            narration_text = meta.get("narration", "").strip()  # ✅ Bug 3 修正
            audio_bytes    = None
            music_bytes    = None
