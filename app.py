...
def build_scene_prompt(scene: dict, characters: list) -> str:
    ...
    # 方案 C 新增：camera 和 pacing 融入 prompt
    ...
    # ==================================================
        ...
        if st.session_state.get("script"):
            ...
            for i, s in enumerate(storyboard):
                ...
                label     = f"{'🔥 ' if is_climax else ''}分鏡 {i+1}｜{s.get('location','')}｜{s.get('camera','')}｜節奏：{s.get('pacing','')}"
            ...
            if st.button(
                ...
                # ✅ 角色快照（用 appearance 欄位）
                chars_snapshot = [
                    {
                        "name":       c.get("name", ""),
                        "appearance": c.get("appearance", ""),
                        "desc":       c.get("appearance", ""),  # 相容 build_scene_prompt
                        "images_b64": [],                        # 模式 C 無圖片
