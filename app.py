...
def render_last_result():
    ...
    if result.get("errors"):
        with st.expander(f"⚠️ 本次錯誤記錄（{len(result['errors'])} 筆）", expanded=True):
...
def run_parallel_video_generation(
    ...
    def generate_one(scene, idx):
        try:
            ...
            # ✅ 修復：所有出場角色描述都進 prompt，ref_img 用第一個有照片的角色
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        if voice_future:
            try:
                ...
                err_msg = f"🎙️ 語音生成失敗：{str(e)[:80]}"
                audio_status.error(err_msg)
                errors.append(err_msg)   # ✅ 記錄錯誤，不中斷流程
        ...
        if music_future:
            try:
                ...
                err_msg = f"🎵 音樂生成失敗：{str(e)[:80]}"
                music_status.error(err_msg)
                errors.append(err_msg)   # ✅ 記錄錯誤，不中斷流程

    return audio_bytes, music_bytes, errors   # ✅ 多回傳 errors
...
def run_gdrive_upload(
    ...
    try:
        ...
        err = f"❌ Google Drive 上傳整體失敗：{str(e)[:80]}"
        st.error(err)
        errors.append(err)
...
def main():
    ...
    if mode == "🎥 單場景模式":
            ...
            with col_s3:
                if duration_type == "快速選擇":
                    ...
                    video_duration = st.number_input(
                        "自訂秒數", min_value=3, max_value=60,
                        value=5, step=1, key="single_dur_custom"
                    )
        ...
        if st.button("🚀 開始生成", type="primary", use_container_width=True, key="single_gen"):
            ...
            st.rerun()  # ✅ rerun 後由 render_last_result() 固定顯示
