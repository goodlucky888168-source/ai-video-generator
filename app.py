...
# ==================== OpenAI 分析 ====================
def analyze_prompt(user_input, openai_key):
    headers = {
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": """你是影片製作助手。根據使用者描述，回傳JSON格式：
{
...
def generate_video(video_prompt, kling_access, kling_secret):
    ...
    for _ in range(60):
        ...
        if status == "succeed":
            return poll.json()["data"]["task_result"]["videos"][0]["url"]
        elif status == "failed":
            return None
    return None
...
# ==================== 主介面 ====================
def main():
    st.set_page_config(page_title="AI 影片生成器", page_icon="🎬")
    ...
    if st.button("🚀 開始生成", type="primary"):
        ...
        with st.spinner("🤖 AI 分析中..."):
            try:
                result = analyze_prompt(user_input, openai_key)
                st.success("✅ 分析完成")
                st.json(result)
