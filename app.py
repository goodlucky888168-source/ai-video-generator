...
def add_to_history(record_type: str, title: str, data: dict, error_msg: str = None):
    ...
    Args:
        record_type: "single" | "multi" | "script"
        title: 記錄標題
        data: 包含 videos, audio, music 的字典
        error_msg: 錯誤訊息 (如有)
    """
    record = {
...
def run_parallel_video_generation(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for f in concurrent.futures.as_completed(future_map):
            ...
            if err:
                scene_status[idx].error(f"❌ 分鏡 {idx+1} 失敗: {err}")
            else:
...
def _render_mode_multi(keys, ev, em, eg):
    ...
    if st.session_state.storyboard:
        st.info("💡 多人分鏡將自動切換為文字模式，以確保背景正確且多人同時出現。")
        ...
        if st.button("🚀 生成所有分鏡影片", type="primary", use_container_width=True):
            try:
                ...
                add_to_history(
                    "multi",
                    f"多角色分鏡 ({len(st.session_state.characters)} 人)",
                    {
...
def _render_mode_script(keys, ev, em, eg):
    ...
    with col_idea:
        idea = st.text_area(
            "💡 故事點子",
            placeholder="例如：外星人在地球的海邊第一次看到冰淇淋",
            height=120
        )
