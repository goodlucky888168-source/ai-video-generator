...
def build_scene_prompt(scene: dict, characters: list) -> str:
    ...
    # camera / pacing 融入 prompt（模式 C 專屬欄位，模式 B 會是空字串）
...
def render_history():
    ...
    with st.expander(
        ...
        for i, record in enumerate(reversed(st.session_state.history)):
            ...
            st.caption(
                f"模式：{record['mode']} ｜ 旁白：{record.get('narration','')[:30]}"
            )
    ...
    # ==================================================
            ...
            for i, s in enumerate(st.session_state.storyboard):
                with st.expander(
                    ...
                    for lbl in scene_chars:
                        idx = ord(lbl) - 65
                        if 0 <= idx < len(selected_chars):
                            c = selected_chars[idx]
                            if c.get("images_b64"):
        ...
        if st.button(
            ...
            st.info(
                ...
                f"📸 {has_photo_count} 個角色使用照片（image2video） ｜ "
                f"🤖 {len(chars_snapshot) - has_photo_count} 個角色使用 AI 描述（text2video）"
            )
    ...
    # ==================================================
        ...
        if st.button("🎬 生成劇本", use_container_width=True, key="script_gen"):
            if not idea.strip():
                ...
                # 先把照片轉 base64 存進 session_state（rerun 後仍有效）
                photos = {}
            ...
            for i, s in enumerate(storyboard):
                ...
                with st.expander(label, expanded=True):
                    ...
                    with col_b:
                        ...
                        st.text_input(
                            "對話（中文）",
                            value=s.get("dialogue", ""),
                            key=f"script_dial_{i}"
                        )
                    # 分鏡參考來源標示
                    scene_chars = s.get("characters", [])
                    photo_info  = []
                    for lbl in scene_chars:
                        if lbl in char_photos:
                            photo_info.append(f"角色 {lbl} 📸")
                        else:
                            photo_info.append(f"角色 {lbl} 🤖")
                    if photo_info:
                        st.caption(f"參考來源：{' ｜ '.join(photo_info)}")
            ...
            if st.button(
                ...
                for i, c in enumerate(script.get("characters", [])):
                    ...
                    chars_snapshot.append({
                        ...
                        "images_b64": [char_photos[lbl]] if lbl in char_photos else [],
                        "label":      lbl
                    })
                ...
                # 生成前摘要
                has_photo_count = sum(1 for c in chars_snapshot if c["images_b64"])
                st.info(
                    f"📊 共 {total_scenes} 個分鏡 ｜ "
                    f"📸 {has_photo_count} 個角色使用照片（image2video） ｜ "
                    f"🤖 {len(chars_snapshot) - has_photo_count} 個角色使用 AI 描述（text2video）"
                ...
                if enable_gdrive:
                    ...
                    with st.spinner("上傳中..."):
                        run_gdrive_upload(
                            ...
                            keys,
                            prefix=f"script_{script.get('title','')[:10]}"
                        )
